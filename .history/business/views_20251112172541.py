...
class CompanyCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Use unsaved Company instance so empty_form & management_form render with proper prefix
        empty_company = Company()

        if self.request.method == 'POST':
            context['meeting_formset'] = MeetingFormSet(self.request.POST, self.request.FILES, instance=empty_company)
            context['followup_formset'] = FollowupFormSet(self.request.POST, self.request.FILES, instance=empty_company)
            context['comment_formset'] = CommentFormSet(self.request.POST, self.request.FILES, instance=empty_company)
            context['voice_formset'] = VoiceFormSet(self.request.POST, self.request.FILES, instance=empty_company)
            context['visit_formset'] = VisitFormSet(self.request.POST, self.request.FILES, instance=empty_company)
        else:
            # Important: pass instance=empty_company for GET so management_form & empty_form are correct
            context['meeting_formset'] = MeetingFormSet(instance=empty_company)
            context['followup_formset'] = FollowupFormSet(instance=empty_company)
            context['comment_formset'] = CommentFormSet(instance=empty_company)
            context['voice_formset'] = VoiceFormSet(instance=empty_company)
            context['visit_formset'] = VisitFormSet(instance=empty_company)

        return context

    def form_valid(self, form):
        # set owner fields on parent
        form.instance.created_by = self.request.user
        form.instance.updated_by = self.request.user

        # Save Company first (so inline instances can be attached)
        response = super().form_valid(form)
        company = self.object

        # instantiate formsets bound to the saved company
        ...
        # helper to save each formset consistently
        def save_fs(fs, is_visit=False):
            if not fs.is_valid():
                return False, fs.errors
            instances = fs.save(commit=False)
            for obj in instances:
                # attach the company if visit or if formset doesn't already have company
                if is_visit and not getattr(obj, 'company', None):
                    obj.company = company
                # set common audit fields where present
        ...
        # Save each formset and collect errors (if any)
        for fs, visit_flag in [(meeting_fs, False), (followup_fs, False),
                               (comment_fs, False), (voice_fs, False), (visit_fs, True)]:
            ok, errors = save_fs(fs, is_visit=visit_flag)
            if not ok:
                # If any inline invalid, delete the created company (to avoid partials)
                # and re-render with errors by returning form invalid.
                company.delete()
                form.add_error(None, "One of the inline formsets had errors.")
...
class CompanyUpdateView(LoginRequiredMixin, UpdateView):
    model = Company
    form_class = CompanyForm
    template_name = 'business/company_edit_styled.html'
    ...
    def form_valid(self, form):
        ...
        # same saving logic as create view
        for fs in (meeting_fs, followup_fs, comment_fs, voice_fs, visit_fs):
            if fs.is_valid():
                ...
                for inst in instances:
                    ...
                    # ensure company FK present if applicable
                    if hasattr(inst, 'company') and not getattr(inst, 'company', None):
                        inst.company = company
                    inst.save()
                # Important: call fs.save() to process deletes / newly-added forms and m2m
                fs.save()
        return response
