...
def edit_company_meeting(request, pk):
    ...
    # Get related company from the meeting
    company = meeting.company  

    if request.method == 'POST':
        # Bind both forms
        meeting_form = BusinessMeetingForm(request.POST, instance=meeting)
        ...
        if meeting_form.is_valid() and company_form.is_valid():
            ...
            messages.success(request, f"✅ Meeting #{meeting.id} and Company '{company.company_name}' updated successfully!")
            return redirect('company_meetings')
        else:
            messages.error(request, "❌ Please fix the form errors below.")
    else:
        # Initial form load
        meeting_form = BusinessMeetingForm(instance=meeting)
