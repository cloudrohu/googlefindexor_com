...
def edit_company_meeting(request, pk):
    ...
    # Get the meeting instance and related company
    meeting = get_object_or_404(CompanyMeeting, pk=pk)
    company = meeting.company

    if request.method == 'POST':
        # ✅ Use prefixes to avoid field name conflicts
        meeting_form = BusinessMeetingForm(request.POST, instance=meeting, prefix='meeting')
        ...
        if meeting_form.is_valid() and company_form.is_valid():
            ...
            meeting_obj.company = company_obj  # reattach (just in case)
            meeting_obj.save()
        ...
        # Initial load with prefilled forms (with prefixes)
